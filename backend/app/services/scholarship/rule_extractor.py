...
class ScholarshipRuleExtractionError(
    RuntimeError
):
    """
    Lỗi khi không thể trích xuất bộ luật
    học bổng hợp lệ.
    """


class ScholarshipRuleExtractor:
    """
    Chuyển các đoạn tri thức do RAG truy xuất
    thành ScholarshipRuleSet có cấu trúc.
    ...
    def build_prompt(
        ...
        *,
        ...
        """
        Tạo prompt trích xuất luật học bổng
        từ các chunk trong kho tri thức.
        """
...
Nhiệm vụ:
Chuyển các điều kiện học bổng xuất hiện rõ ràng
...
2. Không tự đặt ngưỡng GPA, điểm rèn luyện hoặc
   điều kiện học bổng.
...
11. Nếu tài liệu không nêu rõ các giá trị cho phép
    hoặc bị loại trừ thì không tạo luật đó.
12. Với operator số như greater_than_or_equal
    hoặc less_than_or_equal, required_value phải
    là số.
13. Không đưa cùng một điều kiện vào nhiều luật
    trùng lặp.
14. category của nguồn phải là "hoc_bong".
15. scholarship_type chỉ điền khi tài liệu xác định
    rõ loại học bổng.
...
{{
  ...
  "rules": [
    {{
      "code": "minimum_gpa",
      "name": "Điểm trung bình tích lũy tối thiểu",
      "field_name": "gpa",
      "operator": "greater_than_or_equal",
      "required_value": 3.2,
      "accepted_values": [],
      "failure_message": "GPA chưa đạt yêu cầu.",
      "recommendation": "Cần cải thiện GPA.",
      "is_required": true,
      "confidence": 0.95,
      "source": {{
        "document_name": "hoc_bong.pdf",
        "page_number": 1,
        "chunk_id": "chunk_1",
...
""".strip()
    ...
    def _extract_json(
        ...
        if not response_text:
            raise ScholarshipRuleExtractionError(
        ...
        try:
            ...
            try:
                ...
                raise (
                    ScholarshipRuleExtractionError(
    ...
    def _parse_rule_set(
        ...
        """
        Parse nội dung LLM thành ScholarshipRuleSet.
        """
        ...
        try:
            return ScholarshipRuleSet.model_validate(
                raw_rule_set
            )
    ...
    def _repair_rule_set(
        ...
        *,
        ...
        Chỉ sửa tối đa một lần.
        """

        repair_prompt = f"""
Bạn đã sinh một bộ luật xét học bổng dưới dạng
...
12. category của source phải là "hoc_bong".
""".strip()
    ...
    def extract(
        ...
        *,
        ...
        """
        Trích xuất và kiểm tra bộ luật học bổng.
        """
        ...
        except ScholarshipRuleExtractionError as error:
            try:
