...
class LLMInsightWriter:
    ...
    def _request_llm(
        ...
        """
        Gửi insight đã tính toán qua LLMManager.

        Thứ tự xử lý:
        1. Gemini API trực tiếp
        2. Gemini qua OpenRouter
        3. Nếu tất cả thất bại, ném lỗi để
        LLMInsightWriter dùng local fallback
        """
        ...
        if not result.success:
            raise RuntimeError(
                result.error
                or "Không có LLM provider nào khả dụng."
            )
